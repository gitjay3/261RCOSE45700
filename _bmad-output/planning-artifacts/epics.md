...
시스템이 크롤러 큐에서 게시글을 소비하고 **OpenAI 멀티모달 LLM(GPT-4o/4.1) 단일 호출**로 자동 분류하여, 신뢰도·판단 근거·**Tier(T1/T2/T3/T4)**를 포함한 탐지 결과를 RDS에 저장한다. T1 즉시 알림, Tier별 차등 retry/보존 정책 적용. Epic 3 완료 기준에 Tier별 Precision/Recall 측정 + 게시글당 평균 비용 측정 포함.

> **2026-05-27 PIVOT (Correct Course)** — VARCO Translation+LLM 2단 → OpenAI 멀티모달 LLM 단일 호출 + Tier 차등 처리.
>
> **스토리 변경 요약:**
...
### Story 3.5: Tier별 배치 처리 시간 + Tier별 정확도 + 비용 사전 측정 (2026-05-27 PIVOT — Tier 확장)

QA 담당자로서,
Epic 3 완료 시점에 1회 배치 처리 시간과 **Tier별 Precision/Recall + 게시글당 평균 비용**을 측정하기를 원한다,
그래서 최종 목표치(배치 ≤ 30분, T1 Recall ≥ 0.85, Precision ≥ 0.85, 비용 ≤ $0.005/post) 달성 가능성을 조기에 검증할 수 있다.

**Acceptance Criteria:**

**Given** `tests/fixtures/labels/manual_label_set_v2.csv`의 **≥ 300건 라벨셋(Tier별 ≥ 75건, 이미지 첨부 ≥ 50건)**이 준비된 상태에서
**When** `detection/tests/quality/test_llm_precision_recall_tiered.py`를 실행하면
**Then** 전체 Precision ≥ 0.80을 만족한다 (Epic 3 사전 임계값 — 최종 목표 0.85의 버퍼)
**And** T1 Critical Recall ≥ 0.80을 만족한다 (최종 목표 0.85의 버퍼)
**And** T2 High Recall ≥ 0.65, T3 Medium Recall ≥ 0.50을 만족한다
**And** **Tier별 confusion matrix**(T1/T2/T3/T4 × predicted Tier)가 출력되어 Tier 오분류 패턴이 가시화된다
**And** 게시글당 평균 비용(USD), p95 latency, 이미지 첨부 유/무 분리 집계 결과가 출력된다
**And** 이 측정은 `llm_mock.py` 기반 오프라인 검증이며, 실제 OpenAI API 결과와의 차이는 `docs/quality-gate-epic3.md`에 명시한다
**And** 라이브 검증(소규모 ≥30건, 실제 OpenAI 호출) 결과도 별도 기록 — `docs/quality-gate-epic3-live.md`
**And** 크롤링 시작부터 RDS 저장 완료까지의 1회 배치 처리 시간이 로그에서 측정되어 ≤ 30분임을 확인한다 (NFR3)
**And** 측정 결과가 `docs/quality-gate-epic3.md`에 기록된다

### Story 3.6: Tier 기반 알림 및 보존 정책 (2026-05-27 PIVOT — 신규)
