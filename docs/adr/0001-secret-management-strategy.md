...
Story 5-2 (GitHub Actions 완전 통합 CI/CD 파이프라인) 구현 중 4개 컨테이너(crawler / detection / api / dashboard)가 사용할 시크릿 4종을 어떻게 EC2에 안전하게 전달할지 결정 필요.

**시크릿 4종**: `VARCO_API_KEY`, `DB_PASSWORD`, `RDS_ENDPOINT`, `VARCO_API_BASE_URL`
...
6. **시크릿 회전 주기 > 프로젝트 수명** (VARCO API key는 발급받고 끝, RDS endpoint는 인프라 재구축 전까지 불변)
...
핵심 메커니즘:
- 시크릿 파일: EC2의 `/opt/app/secrets/varco_api_key`, `/opt/app/secrets/db_password` (chmod 600, owner root:root) — `docs/deployment.md` §2.6 절차로 SSH 1회 작성
- `.env` 파일: EC2의 `/opt/app/.env` (chmod 600, root:root) — 비-시크릿 환경변수
- Docker 노출: `infra/compose.prod.yml`의 `secrets:` 키워드로 컨테이너 내부 `/run/secrets/<name>` tmpfs read-only mount
- ENV 변환: `infra/docker-secret-shim.sh`가 `/run/secrets/<name>` → 대문자 ENV 변환 (앱 코드는 `os.environ["VARCO_API_KEY"]` 그대로 사용)
- 검증·안전 가드 (이미 `deploy.yml`에 구현됨):
  1. **IMAGE_TAG fail loud**: hex-only + length 7~40 검증, 위반 시 `exit 1` ([deploy.yml:197-207](../../.github/workflows/deploy.yml#L197-L207))
  2. **Cold-start 가드**: 이전 SHA marker (`/opt/app/IMAGE_TAG`) 없으면 `:latest`로 무한 롤백 거부 + `exit 4` ([deploy.yml:273-280](../../.github/workflows/deploy.yml#L273-L280))
  3. **Container healthcheck 폴링**: Dockerfile `HEALTHCHECK` 기반 + 180s deadline 동안 5개 서비스 healthy 확인, 실패 시 자동 롤백 + 롤백 후 재검증 ([deploy.yml:245-302](../../.github/workflows/deploy.yml#L245-L302))
...
| 대안 | 장점 | 단점 | 결과 |
|------|------|------|------|
| **A. Docker secrets + EC2 SSH 수동** | 코드 변경 0 LOC, `secrets:` tmpfs 격리(`ps`/`/proc/<pid>/environ` 노출 차단), GHA에 시크릿 도달 0, 학생 IAM·외부 SaaS 회피 정책 모두 호환 | 시크릿 회전 시 EC2 SSH 필요, Source of Truth = EC2 디스크 (인스턴스 분실 시 복구 0%), audit = `auth.log`만 | **채택** |
...
| D. `.env` 단일 파일 + `env_file:` | 12-factor 정렬, 운영 단순성(단일 파일), LOC -50 | 현재 작성된 `compose.prod.yml` `secrets:` 블록 + `docker-secret-shim.sh` 폐기 = 작동 자산 되돌림, `env_file:`은 컨테이너 내부 `env`/`docker inspect`에 시크릿 평문 노출 → 보안 격리 -1단계, D-day 회귀 risk | 기각 — Amelia "Config ≠ Secret, VARCO key 노출 시 과금 폭탄" + 보안 격리 후퇴 |
...
- **코드 변경 0 LOC**: `infra/compose.prod.yml` (123 LOC) + `infra/docker-secret-shim.sh` 이미 작성된 자산 유지 → 회귀 risk 최저
- **보안 격리 +1**: Docker `secrets:` tmpfs read-only mount → `docker inspect` / `ps` / `/proc/<pid>/environ` 모두 시크릿 평문 미노출
...
- **회전 시 컨테이너 재기동만 필요**: shim이 파일 변화 감지하여 ENV 갱신 가능
- **6 layer defense-in-depth**: 파일 권한(600 root:root) + Docker tmpfs + ENV 노출 회피(shim) + git 격리(.gitignore) + Network 격리(fail2ban) + Audit(auth.log)
...
### 관련 파일
- `infra/compose.prod.yml` (`secrets:` 블록, 123 LOC)
- `infra/docker-secret-shim.sh` (`/run/secrets/` → ENV 변환)
- `.github/workflows/deploy.yml` (deploy job — IMAGE_TAG 검증 L197-207, cold-start 가드 L273-280, container healthcheck 폴링 L245-256, 자동 롤백 + 재검증 L282-302)
